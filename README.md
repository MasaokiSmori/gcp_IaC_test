...
* VPC SC 境界: ERPソースプロジェクトを含む形でアクセスポリシーを設計し、ERP→prod間のデータ連携を境界内で完結させる。
...
|--------|--------------------------|----------|---------|
| Senior DS | g-datascience-snr@[workspace-domain] | BigQuery Admin (Dataset限定) | Owner 相当 |
| Junior DS | g-datascience-jnr@[workspace-domain] | BQ Data Viewer, Job User, prod_test Dataset編集 | Editor 相当 |
...
③ 【以下、修正が完了するまで繰り返す】
   ...
   (stg も enable_private_endpoint=true のため IAP トンネル経由でアクセス)
